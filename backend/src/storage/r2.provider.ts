...
import type { StorageProvider, BucketCredentials, UsageStats } from './provider.interface'
...
export class CloudflareR2Provider implements StorageProvider {
  ...
  async createBucket(userId: string): Promise<BucketCredentials> {
    ...
    try {
      ...
      if (err.name === 'BucketAlreadyExists') throw new Error(`Bucket collision: ${bucketName}`)
      throw err
    }

    const tokenRes = await api.post(`/accounts/${config.cloudflare.accountId}/r2/tokens`, {
      name: `solstore-user-${userId.slice(0, 8)}`,
      policies: [{
        effect: 'allow',
        resources: { [`com.cloudflare.api.account.r2.bucket.${config.cloudflare.accountId}.${bucketName}`]: '*' },
        actions: ['admin'],
      }],
    })
  ...
  async suspendBucket(_bucketName: string, tokenId?: string): Promise<void> {
    if (!tokenId) return
    const api = this.cfApi()
    await api.delete(`/accounts/${config.cloudflare.accountId}/r2/tokens/${tokenId}`)
    console.log(`[r2] Token revoked: ${tokenId}`)
  }

  async reactivateBucket(bucketName: string, userId: string): Promise<BucketCredentials> {
    const api = this.cfApi()
    const tokenRes = await api.post(`/accounts/${config.cloudflare.accountId}/r2/tokens`, {
    ...
    if (!tokenRes.data?.success) throw new Error('Failed to create new R2 token on reactivation')

    const tokenData = tokenRes.data.result
    console.log(`[r2] Bucket reactivated: ${bucketName}`)

    return {
      bucketName, provider: 'R2',
      accessKey: tokenData.accessKeyId,
      secretKey: tokenData.secretAccessKey,
      endpoint: `https://${config.cloudflare.accountId}.r2.cloudflarestorage.com`,
      region: 'auto',
  ...
  async getUsageBytes(bucketName: string): Promise<UsageStats> {
    ...
    do {
      ...
      for (const obj of res.Contents ?? []) { totalBytes += obj.Size ?? 0; objectCount++ }
