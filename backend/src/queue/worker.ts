...
import { provisionStorage, suspendStorage, reactivateStorage } from '../services/provisioner.service'
...
async function processJob(job: Job): Promise<void> {
  ...
  switch (job.name) {
    ...
    case JOB_NAMES.REFRESH_USAGE: {
      ...
      await recordUsageSnapshot({ userId: data.userId, storageBytes: usage.storageBytes, objectCount: usage.objectCount })
      break
    }
    case JOB_NAMES.SEND_LOW_BAL_EMAIL: {
      const data = job.data as SendLowBalanceEmailJob
      await checkAndNotifyLowBalance(data.userId, data.balanceUsd, data.userEmail)
      break
    }
    default:
...
export function startWorker(): Worker {
  ...
  _worker = new Worker('storage', processJob, { connection: getWorkerConnection(), concurrency: 3 })

  _worker.on('completed', (job) => console.log(`[worker] Done: ${job.name} (${job.id})`))
...
async function logFailedJob(job: Job, err: Error): Promise<void> {
  ...
  await prisma.indexerState.upsert({
    where: { key },
    create: { key, value: JSON.stringify({ jobName: job.name, jobId: job.id, error: err.message, failedAt: new Date().toISOString() }) },
