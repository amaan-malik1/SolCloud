...
export async function provisionStorage(event: {
  userId: string; txSignature: string; solAmount: number; usdAmount: number; solPriceUsd: number
}): Promise<void> {
  const { userId } = event
  ...
  console.log(`✅ Storage provisioned for user ${userId} → ${credentials.bucketName}`)
}
...
export async function reactivateStorage(userId: string): Promise<void> {
  ...
  // Clear grace period
  await prisma.indexerState.deleteMany({ where: { key: `grace:${userId}` } }).catch(() => {})
...
export async function suspendStorage(userId: string): Promise<void> {
  ...
  const provider = getStorageProvider()
  try {
    await provider.suspendBucket(bucket.bucketName, bucket.cfTokenId ?? undefined)
  } catch (err) {
