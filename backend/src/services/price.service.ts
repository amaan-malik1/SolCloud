...
interface PriceCache { usd: number; fetchedAt: number }
...
export async function getSolPriceUsd(): Promise<number> {
  ...
  try {
    ...
    if (!price || typeof price !== 'number') throw new Error('Invalid price response')
...
export async function solToUsd(solAmount: number): Promise<{ usdAmount: number; solPriceUsd: number }> {
