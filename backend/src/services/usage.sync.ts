...
export async function syncBucketUsage(userId: string, bucketName: string): Promise<{ storageBytes: number; objectCount: number }> {
...
export function startUsageSync(): void {
  ...
  console.log(`📊 Usage sync started — interval: ${SYNC_INTERVAL_MS / 60000} minutes`)
  syncAllBuckets().catch(console.error)
  syncHandle = setInterval(() => syncAllBuckets().catch(console.error), SYNC_INTERVAL_MS)
}
...
export function getUsageSyncStatus() {
  return {
    ...
    nextSyncAt: syncHandle && lastSyncAt ? new Date(lastSyncAt.getTime() + SYNC_INTERVAL_MS).toISOString() : null,
