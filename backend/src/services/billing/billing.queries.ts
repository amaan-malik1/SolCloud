...
export async function recordBillingCharge(data: {
  userId: string; amountUsd: number; storageBytes: number; gbHours: number; periodStart: Date; periodEnd: Date
}) {
...
export async function recordUsageSnapshot(data: { userId: string; storageBytes: number; objectCount: number }) {
  return prisma.usageSnapshot.create({
    data: { userId: data.userId, storageBytes: BigInt(data.storageBytes), objectCount: data.objectCount },
  })
}
...
export async function getUserBillingHistory(userId: string, limit = 30) {
  return prisma.billingRecord.findMany({
    ...
    take: limit,
    select: { id: true, amountUsd: true, storageBytes: true, gbHours: true, periodStart: true, periodEnd: true, createdAt: true },
  })
}
