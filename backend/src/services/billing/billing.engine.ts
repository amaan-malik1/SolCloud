...
import { recordBillingCharge, recordUsageSnapshot, getActiveBucketsWithUsers } from './billing.queries'
import { queueSuspendStorage, queueSendLowBalanceEmail } from '../../queue/queues'

export interface BillingRunResult {
  processedUsers: number; totalCharged: number; suspendedUsers: number
  skippedUsers: number; errors: string[]; startedAt: Date; completedAt: Date
}
...
export async function startGracePeriod(userId: string, userEmail: string): Promise<void> {
  const key = `grace:${userId}`
...
async function billUser(
  userId: string, bucketName: string, currentBalanceUsd: number, periodStart: Date, periodEnd: Date
  ...
  try {
    ...
    const bucket = await prisma.bucket.findFirst({ where: { bucketName }, select: { storageBytes: true } })
    usage.storageBytes = Number(bucket?.storageBytes ?? 0)
  }

  await updateBucketStorage(userId, usage.storageBytes)
  await recordUsageSnapshot({ userId, storageBytes: usage.storageBytes, objectCount: usage.objectCount })
  ...
  if (currentBalanceUsd <= BILLING.SUSPENSION_THRESHOLD) {
    ...
    if (!inGrace) {
      ...
      if (user) {
        ...
        await queueSendLowBalanceEmail({ userId, userEmail: user.email, balanceUsd: 0, thresholdUsd: 0 })
      }
  ...
  await recordBillingCharge({ userId, amountUsd: actualCharge, storageBytes: usage.storageBytes, gbHours: calc.gbUsed * 24, periodStart, periodEnd })

  const newBalance = currentBalanceUsd - actualCharge
  if (newBalance <= 2) {
    const user = await prisma.user.findUnique({ where: { id: userId }, select: { email: true } })
    if (user) await queueSendLowBalanceEmail({ userId, userEmail: user.email, balanceUsd: newBalance, thresholdUsd: 2 })
  }
...
export async function runBillingCycle(): Promise<BillingRunResult> {
  ...
  const result: BillingRunResult = {
    processedUsers: 0, totalCharged: 0, suspendedUsers: 0,
    skippedUsers: 0, errors: [], startedAt, completedAt: new Date(),
  }
  ...
  for (const bucket of activeBuckets) {
    const balanceUsd = bucket.user.balance ? Number(bucket.user.balance.amountUsd) : 0
    try {
      const { charged, suspended, skipped } = await billUser(bucket.userId, bucket.bucketName, balanceUsd, periodStart, periodEnd)
  ...
  console.log(`[billing] Cycle complete — Processed: ${result.processedUsers} | Charged: $${result.totalCharged.toFixed(6)} | Suspended: ${result.suspendedUsers}`)
  return result
}
