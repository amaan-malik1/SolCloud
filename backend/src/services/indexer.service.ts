...
async function processTransaction(signature: string): Promise<void> {
  ...
  const platformIndex = accounts.findIndex(
    (a) => a.pubkey.toBase58() === platformAddress,
  );
  ...
  try {
    ...
    // Prisma unique constraint = P2002, postgres = 23505
  ...
  console.log(
    `✅ Payment processed: ${solAmount} SOL ($${usdAmount}) → user ${userId}`,
  );
...
async function poll(): Promise<void> {
  ...
  try {
    ...
    // Ignore "transaction not found" — happens briefly after saving last signature
    if (err?.code === -32020) {
      console.log("[indexer] RPC node catching up, retrying next poll...");
      return;
    }
...
export async function startIndexer(): Promise<void> {
  if (isRunning) return;
  console.log(
    `🔍 Indexer starting — polling every ${config.solana.pollIntervalMs}ms`,
  );
  isRunning = true;
  await poll();
  intervalHandle = setInterval(async () => {
    await poll();
  }, config.solana.pollIntervalMs);
}
