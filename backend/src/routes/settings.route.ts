...
router.get('/profile', authMiddleware, async (req: Request, res: Response) => {
  try {
    const user = await prisma.user.findUnique({
      ...
      select: {
        id: true, email: true, walletAddress: true, createdAt: true,
...
router.patch('/wallet', authMiddleware, async (req: Request, res: Response) => {
  ...
  if (!walletAddress) { res.status(400).json({ error: 'Wallet address is required' }); return }
  const BASE58_REGEX = /^[1-9A-HJ-NP-Za-km-z]{32,44}$/
  if (!BASE58_REGEX.test(walletAddress)) { res.status(400).json({ error: 'Invalid Solana wallet address' }); return }
  try {
    await prisma.user.update({ where: { id: req.user!.userId }, data: { walletAddress } })
...
router.patch('/password', authMiddleware, async (req: Request, res: Response) => {
  ...
  if (newPassword === currentPassword) { res.status(400).json({ error: 'New password must be different' }); return }
  try {
...
router.get('/notifications', authMiddleware, async (req: Request, res: Response) => {
  try {
    const row = await prisma.indexerState.findUnique({ where: { key: `notif:${req.user!.userId}` } })
...
router.patch('/notifications', authMiddleware, async (req: Request, res: Response) => {
  ...
  try {
    await prisma.indexerState.upsert({
      where: { key: `notif:${req.user!.userId}` },
      create: { key: `notif:${req.user!.userId}`, value: JSON.stringify({ lowBalanceThreshold, emailAlerts }) },
...
router.delete('/account', authMiddleware, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (bucket && bucket.status === 'ACTIVE') {
      await suspendStorage(req.user!.userId).catch(() => {})
    }
