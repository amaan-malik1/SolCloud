...
export function usePaymentConfirmation() {
  ...
  const startPolling = useCallback(async (previousBalance: number) => {
    setStatus('waiting')
    startTimeRef.current = Date.now()

    pollingRef.current = setInterval(async () => {
      if (Date.now() - startTimeRef.current > 120_000) { stopPolling(); setStatus('timeout'); return }
      try {
  ...
  const reset = useCallback(() => { stopPolling(); setStatus('idle'); setCreditedUsd(null) }, [stopPolling])
