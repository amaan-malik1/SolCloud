...
export function useNotificationPrefs() {
  return useQuery({ queryKey: ['notifications'], queryFn: settingsApi.getNotifications })
}
...
export function useUpdateWallet() {
  ...
  return useMutation({
    ...
    onSuccess: () => { toast.success('Wallet address linked'); queryClient.invalidateQueries({ queryKey: ['profile'] }) },
    onError: (err: any) => toast.error(err?.response?.data?.error ?? 'Failed to update wallet'),
  })
}
...
export function useDeleteAccount() {
  ...
  return useMutation({
    ...
    onSuccess: () => { toast.success('Account deleted'); clearAuth(); window.location.href = '/' },
